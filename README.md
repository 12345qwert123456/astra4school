...
### Системные утилиты:
- **ЕСПД Прокси** - прокси-сервер для работы с ЕСПД

### Дополнительно для ПК учителя:
- **Assistant** - помощник учителя для управления классом
...
1. Загрузите два файла:
   ...
   - `.env.example` - файл конфигурации (конфиденциальный)
...
2. Настройте `.env`:
   ...
   # По умолчанию укажите GIT_USERNAME='12345qwert123456', GIT_REPO='astra4school'
   # Если был сделан форк, то укажите свои данные
   ```
...
4. Выберите тип установки:
   - **1** - ПК учителя (с Assistant и дополнительным ПО)
   - **2** - ПК ученика (стандартная установка)

Скрипт автоматически:
- Проверит подключение к интернету и GitHub
- Установит Git (если необходимо)
- Клонирует репозиторий со всеми файлами
- Выполнит установку всех компонентов в зависимости от выбранного режима

## 🔧 Как это работает

1. **run.sh** - загрузчик:
   - Проверяет права администратора
   - Предлагает выбрать тип установки (учитель/ученик)
   ...
   - Запускает основной скрипт с выбранным режимом

2. **setup-main.sh** - основной скрипт:
   - Получает режим установки (teacher/student)
   - Настраивает SSH ключи
   - Включает репозитории
   - Создает пользователей
   - Устанавливает программное обеспечение
   - Устанавливает Assistant (только для режима teacher)
   - Копирует ярлыки на рабочий стол
